...
using Eevee.Random;
using UnityEngine;
using EPlane = Eevee.Fixed.Plane;
using EQuaternion = Eevee.Fixed.Quaternion;
...
/// 确定性库示例代码
/// </summary>
internal sealed class MathSample : MonoBehaviour
{
    public int times;
    ...
    private void Update()
    {
        for (int i = 0; i < times; ++i)
        {
            ...
            var quaternion2 = (EQuaternion)URandom.rotation;
            ...
            var plane0 = new EPlane(in vec3D0, num2);
            var plane1 = new EPlane(in Vector3D.Up, Fixed64.Zero);
            ...
            if (diffMoveTowardsAngle >= _epsilon0001)
                Debug.LogError($"[Sample] MoveTowardsAngle Diff:{diffMoveTowardsAngle} not 0.");
            ...
            if (diffAngleAxis.SqrMagnitude() >= _epsilon0001)
                Debug.LogError($"[Sample] AngleAxis Diff:{diffAngleAxis} not 0.");
            ...
            if (diffV2Angle.Abs() >= _epsilon01)
                Debug.LogError($"[Sample] Angle V2 Diff:{diffV2Angle} not 0.");
            ...
            if (diffV3Angle.Abs() >= _epsilon0001)
                Debug.LogError($"[Sample] Angle V3 Diff:{diffV3Angle} not 0.");
            ...
            if (diffQAngle.Abs() >= _epsilon02)
                Debug.LogError($"[Sample] Angle Q Diff:{diffQAngle} not 0.");
            ...
            if (diffV2SignedAngle.Abs() >= _epsilon01)
                Debug.LogError($"[Sample] SignedAngle V2 Diff:{diffV2SignedAngle} not 0.");
            ...
            if (diffPointToNormalized.SqrMagnitude() >= _epsilon0001)
                Debug.LogError($"[Sample] PointToNormalized Diff:{diffPointToNormalized} not 0.");
            ...
            if (diffNormalizedToPoint.SqrMagnitude() >= _epsilon0001)
                Debug.LogError($"[Sample] NormalizedToPoint Diff:{diffNormalizedToPoint} not 0.");
            ...
            if (diffSLerp.SqrMagnitude() >= _epsilon0001)
                Debug.LogError($"[Sample] SLerp Diff:{diffSLerp} not 0.");
            ...
            if (diffMoveTowards.SqrMagnitude() >= _epsilon0001)
                Debug.LogError($"[Sample] MoveTowards Diff:{diffMoveTowards} not 0.");

            //var uRotateTowards = UQuaternion.RotateTowards((UQuaternion)quaternion0, (UQuaternion)quaternion1, (float)num1);
            var uRotateTowards = (EQuaternion)RotateTowards((UQuaternion)quaternion0, (UQuaternion)quaternion1, (float)num1);
            ...
            if (diffRotateTowards.SqrMagnitude() >= _epsilon01)
                Debug.LogError($"[Sample] RotateTowards Diff:{diffRotateTowards} not 0.");

            var diffSqrMagnitude = UVector3.SqrMagnitude((UVector3)vec3D0) - vec3D0.SqrMagnitude();
            if (diffSqrMagnitude >= _epsilon0001)
                Debug.LogError($"[Sample] SqrMagnitude Diff:{diffSqrMagnitude} not 0.");
            ...
            if (diffClampReflect.SqrMagnitude() >= _epsilon0001)
                Debug.LogError($"[Sample] Reflect Diff:{diffClampReflect} not 0.");
            ...
            if (diffClampProjectOnPlane.SqrMagnitude() >= _epsilon0001)
                Debug.LogError($"[Sample] ProjectOnPlane Diff:{diffClampProjectOnPlane} not 0.");

            var diffGetPoint = ray3D0.GetPoint(num2) - ((URay)ray3D0).GetPoint((float)num2);
            if (diffGetPoint.SqrMagnitude() >= _epsilon0001)
                Debug.LogError($"[Sample] GetPoint Diff:{diffGetPoint} not 0.");
            ...
            if (diffConjugate.SqrMagnitude() >= _epsilon0001)
                Debug.LogError($"[Sample] Conjugate Diff:{diffConjugate} not 0.");

            var fFromToRotation = EQuaternion.FromToRotation(in vec3D0, in vec3D1);
            ...
            if ((fFromToRotation + uFromToRotation).SqrMagnitude() >= _epsilon0000001 && (fFromToRotation - uFromToRotation).SqrMagnitude() >= _epsilon0001)
                Debug.LogError($"[Sample] FromToRotation Add:{fFromToRotation + uFromToRotation}, Diff:{fFromToRotation - uFromToRotation} not 0.");
            ...
            if (diffTranslated >= _epsilon0001)
                Debug.LogError($"[Sample] Translated Diff:{diffTranslated} not 0.");
            ...
            if (diffGetDistanceToPoint >= _epsilon0001)
                Debug.LogError($"[Sample] GetDistanceToPoint Diff:{diffGetDistanceToPoint} not 0.");
            ...
            if (diffClosestPointOnPlane.SqrMagnitude() >= _epsilon0001)
                Debug.LogError($"[Sample] ClosestPointOnPlane Diff:{diffClosestPointOnPlane} not 0.");

            bool diffSameSide = plane0.SameSide(in vec3D0, in vec3D1) == ((UPlane)plane0).SameSide((UVector3)vec3D0, (UVector3)vec3D1);
            if (!diffSameSide)
                Debug.LogError("[Sample] SameSide Diff not 0.");
            ...
            if (diffRayCast.SqrMagnitude().Abs() >= _epsilon0001)
                Debug.LogError($"[Sample] RayCast Diff:{diffRayCast} not 0.");
            ...
            if (bInverse && diffInverse.SqrMagnitude() >= _epsilon0001)
                Debug.LogError($"[Sample] Inverse Diff:{diffInverse} not 0.");
        }
    }
